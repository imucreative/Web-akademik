...
Class Jenis_pembayaran extends CI_Controller {
    ...
    function index() {
		$data['jenis_pembayaran']	= $this->Model_jenis_pembayaran->daftar_jenis_pembayaran();
        $this->template->load('template', 'jenis_pembayaran/list', $data);
    }
    ...
    function delete(){
        ...
        if(!empty($id_jenis_pembayaran)){
			...
			$this->Model_jenis_pembayaran->hapus($id_jenis_pembayaran);
        }
        redirect('jenis_pembayaran');
    }
